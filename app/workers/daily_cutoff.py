...
async def perform_daily_cutoff():
    now_ist = get_ist_now()
    today = now_ist.date()
    ...
    async with AsyncSessionLocal() as db:
        ...
        logger.info(f"[DailyCutoff] Closing {len(rows_to_close)} package inventory rows for {today} at {now_ist.time()}")
...
async def daily_cutoff_loop() -> None:
    logger.info("[DailyCutoff] Worker started.")
    await asyncio.sleep(40) 
    
    while True:
        try:
            await perform_daily_cutoff()
        except Exception as exc:
            logger.error(f"[DailyCutoff] Unexpected error: {exc}")
            
        await asyncio.sleep(CUTOFF_CHECK_INTERVAL_SECONDS)
