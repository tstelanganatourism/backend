...
async def perform_daily_cutoff(ctx):
    ...
    # ── 1. Daily Inventory Cutoff ─────────────────────────────────────────────
    cutoff_key = f"daily_cutoff_done:{today.isoformat()}"
    already_cutoff = await redis.get(cutoff_key)
    
    if not already_cutoff:
        async with AsyncSessionLocal() as db:
            ...
            if rows_to_close or room_rows_to_close:
            ...
            await redis.set(cutoff_key, "1", ex=86400)

    # ── 2. Travel Day SMS Reminders ───────────────────────────────────────────
    # Send to all confirmed bookings travelling today (idempotent via Redis key)
    sms_run_key = f"daily_travel_sms_done:{today.isoformat()}"
    already_sms = await redis.get(sms_run_key)
    ...
    if not already_sms:
        try:
            ...
            async with AsyncSessionLocal() as sms_db:
                ...
                await redis.set(sms_run_key, "1", ex=86400)
                logger.info(f"[TravelSMS] Travel day reminders completed for {today}")
